...
local _MINIMAL_EDUCATION_INVESTMENT_ = 13
local _AVERAGE_EDUCATION_INVESTMENT_ = 14
local _MEDIUM_LARGE_EDUCATION_INVESTMENT_ = 15
local _BIG_EDUCATION_INVESTMENT_ = 16
--
local _CONSUMER_PRODUCT_ORIENTATION_ = 17
local _MIXED_INDUSTRY_ = 18
local _HEAVY_INDUSTRY_EMPHASIS_ = 19

local _FREE_PRESS_ = 20
local _CENSORED_PRESS_ = 21
local _STATE_PRESS_ = 22
local _PROPAGANDA_PRESS_ = 23

local _MINIMAL_TRAINING_ = 24
local _BASIC_TRAINING_ = 25
local _ADVANCED_TRAINING_ = 26
local _SPECIALIST_TRAINING_ = 27
...
function Laws(minister)
	...
	for loGroup in CLawDataBase.GetGroups() do
		...
		elseif loGroupName == "training_laws" then
			loNewLaw = TrainingLaws(ministerTag, ministerCountry, ai, loCurrentLaw)
...
function TrainingLaws(ministerTag, ministerCountry, ai, voCurrentLaw)
	if not IsMajorPower(ministerCountry) then
		local idealLaw = SuggestTrainingLaw(ministerCountry, ai)
		if idealLaw then
			local loNewLaw = CLawDataBase.GetLaw(idealLaw)
